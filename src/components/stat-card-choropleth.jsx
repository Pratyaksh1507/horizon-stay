...
export function StatCardChoropleth({ confirmedStays = [] }) {
  const { worldData, isLoading } = useWorldDataStandalone();
  const { getVisitorColor, getVisitorValue, visitorStats, computeVisitorTrend } = useVisitorStats(confirmedStays);
  ...
  return (
    <Card className="relative w-full gap-0 overflow-hidden py-0">
      <CardHeader
        className="pointer-events-none absolute inset-x-0 top-0 z-10 grid auto-rows-min grid-cols-[1fr_auto] items-start gap-1 border-0 bg-gradient-to-b from-45% from-card to-transparent px-4 py-3 pb-10 shadow-none ring-0">
      ...
      <CardContent className="p-0">
        {isLoading || !worldData ? (
          <StatCardChart className="mx-0 mb-0 min-h-[420px]" size="lg">
            <div
              className="flex h-full min-h-[420px] items-center justify-center text-muted-foreground text-xs">
              Loading map…
            </div>
          </StatCardChart>
        ) : (
          <StatCardChart className="mx-0 mb-0 min-h-[420px]" size="lg">
            <ChoroplethChart aspectRatio="2.5 / 1" className="min-h-[420px] w-full" data={worldData}>
